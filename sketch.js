...
function keyPressed() {
  ...
  if (key === '5') {
    ...
    plantStartYOffset -= 10; // CHANGED: Allow negative values (removed max(0, ...))
    let newPos = calculatePlantBasePosition();
    let offsetY = newPos.baseY - oldPos.baseY;
    adjustPlantPosition(offsetY);
    console.log("plantStartYOffset decreased to:", plantStartYOffset, "(can be negative!)");
  }
  
  if (key === '6') {
    // Calculate the new position and adjust existing plant
    let oldPos = calculatePlantBasePosition();
    plantStartYOffset += 10; // CHANGED: Allow larger positive values
    let newPos = calculatePlantBasePosition();
    let offsetY = newPos.baseY - oldPos.baseY;
    adjustPlantPosition(offsetY);
